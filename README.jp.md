...
**実例（詳細版）:** [Example_CDK20.jp.md](Example_CDK20.jp.md) — 本READMEの各段階を実際のターゲット（ヒトCDK20、UniProt Q8IZL9）でend-to-end実行し、スクリーンショット・インタラクティブHTMLビュー・生データつきで解説しています。
...
`--visualize` を付けると、さらに `pocket_candidates.html` を書き出す。上位
`--visualize-top-n` 個（既定3個）の候補ポケットを受容体構造上で比較する
スタンドアロン（サーバー不要）py3Dmolビューで、半透明カートゥーン表示、
各ポケットの裏打ち残基を不透明スティックで表示、そのポケットのfpocket
アルファスフィア群を単色の球クラスタとして描画（裏打ち残基だけでなく、
空洞そのものが占める実際の体積を表現）、さらに裏打ち残基1つずつに
ラベルを付ける。順位ごとに色分けされるため、rank 1とそれ以降の間の
（しばしば大きな）druggability scoreの差が、数値を読むまでもなく
一目で分かる:
...
ヒトCDK20モデル（UniProt Q8IZL9）で実測: 28個の候補ポケットが検出され、
rank 1（druggability 0.646、既知のATP結合キナーゼポケット — Gly-richループ
...
| `visualize.py` | スタンドアロンpy3Dmol HTML: 各クラスタの代表構造オーバーレイ、および上位候補ポケットの比較（裏打ち残基・アルファスフィア空洞ボリューム・残基ラベル） |
